...
CssX2x.prototype._changeValue = function(unit, value) {
  ...
  return value.replace(reg, function($0, $1) {
    if (unit === 'px') {
      ...
      return config.formTag === 'rem' ? getValue(self._changeValue('px', $1) * config.baseDpr) : getValue($1 * config.baseDpr)
    } else if (unit === 'rem') {
      return config.formTag === 'px' ? getValue($1 / config.tagUnit) : getValue(self._changeValue('px', $1) / config.tagUnit)
    } else {
      return $1
    }
  });
